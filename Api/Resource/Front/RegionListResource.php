...
class RegionListResource extends FrontResource
{
    ...
    {
        if(!isset($uriData['country_id'])) {
            throw NotDefinedException::forParameter('country_id');
        }

        $uri = parent::getUri($shop, $uriData);
        $uri = $uri->withPath($uri->getPath() . '/countries/' . $uriData['country_id'] . '/regions');
